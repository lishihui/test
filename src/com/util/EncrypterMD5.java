 ...
 * 
 * 实现md5加密算法
 * getMD5ofStr(String inbuf)是入口方法，传入一个字符串，返回它加密后的形式
 */
public class EncrypterMD5
{
    ...
    // 这是MD5这个类的标准构造函数，JavaBean要求有一个public static 的并且没有参数的构造函数
    public   EncrypterMD5()
    {
        md5Init();
        return;
    ...
    /*
     * F, G, H ,I 是4个基本的MD5函数，在原始的MD5的C实现中，由于它们是 简单的位运算，可能出于效率的考虑把它们实现成了宏，在java中，我们把它 们 实现成了private方法，名字保持了原来C中的。
     */
    ...
    /*
     * md5Update是MD5的主计算过程，inbuf是要变换的字节串，inputlen是长度， 这个 函数由getMD5ofStr调用，调用之前需要调用md5init，因此把它设计成priva te的
     */
    ...
    /*
     * md5Memcpy是一个内部使用的byte数组的块拷贝函数，从input的inpos开始 把len长度的 字节拷贝到output的outpos位置开始
     */
    ...
    /*
     * md5Transform是MD5核心变换程序，有md5Update调用，block是分块的原始 字节
     */
